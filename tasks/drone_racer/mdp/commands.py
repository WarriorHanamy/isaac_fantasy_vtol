...
class GateTargetingCommand(CommandTerm):
        ...
        if self.cfg.record_fpv:
            if hasattr(self, "out"):
                ...
                if self._current_video_path is not None:
                    print(f"FPV video saved to {self._current_video_path}")
                self.video_id += 1

            video_path = self.fpv_dir / f"fpv_{self.video_id:03d}.mp4"
            self._current_video_path = video_path
            self.out = cv2.VideoWriter(str(video_path), self.fourcc, 100, (1000, 1000))
    ...
    def _update_command(self):
        if self.cfg.record_fpv and hasattr(self, "out"):
            image = self.sensor.data.output["rgb"][0][..., :3]
            if image.dtype == torch.uint8:
                frame = image.cpu().numpy()
            else:
                frame = torch.clamp(image, 0.0, 1.0).mul(255.0).to(dtype=torch.uint8).cpu().numpy()
    ...
    def __del__(self):
        if getattr(self.cfg, "record_fpv", False) and hasattr(self, "out"):
            self.out.release()
            if self._current_video_path is not None:
                print(f"FPV video saved to {self._current_video_path}")
